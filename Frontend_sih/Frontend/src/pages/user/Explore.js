...
const Explore = ({ onArViewClick, isNgoView = false, onProposeEvent }) => {
  const [searchTerm, setSearchTerm] = useState('');
  const [sortBy, setSortBy] = useState('featured');
  const { t } = useLocale();

  const filteredMonasteries = useMemo(() => {
    let monasteries = [...mockMonasteries];
    if (searchTerm) {
    ...
    if (sortBy !== 'featured') {
      monasteries.sort((a, b) => {
        switch (sortBy) {
          case 'rating_asc': return a.rating - b.rating;
          case 'name_asc': return a.name.localeCompare(b.name);
          case 'name_desc': return b.name.localeCompare(a.name);
          case 'rating_desc': default: return b.rating - a.rating;
        }
      });
    }
    return monasteries;
  ...
  return (
    <div className="explore-page">
      {!isNgoView && <EventCarousel />}
      <div className="controls-container">
        <div className="search-bar">
          <FaSearch className="search-icon" />
          <input type="text" placeholder={t('explore.searchPlaceholder')} value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} />
        </div>
        <div className="filter-bar">
          <FaFilter className="filter-icon" />
          <select value={sortBy} onChange={(e) => setSortBy(e.target.value)}>
            <option value="featured">{t('explore.sort.featured')}</option>
            <option value="rating_desc">{t('explore.sort.ratingHigh')}</option>
            <option value="rating_asc">{t('explore.sort.ratingLow')}</option>
            <option value="name_asc">{t('explore.sort.nameAZ')}</option>
            <option value="name_desc">{t('explore.sort.nameZA')}</option>
          </select>
        </div>
      </div>
